"""
Phase 4, Orange: reads chunks_orange.py, embeds each chunk, writes
ingest_orange.sql (documents/chunks + fee_schedules).

Usage:
    source .venv/bin/activate
    python ingest_orange.py
    psql "$DATABASE_URL" -f ingest_orange.sql
"""

import sys

from chunks_orange import CITY, DOCUMENTS, FEE_ROWS
...
select target_city.id, v.permit_type, v.fee_name, v.amount_usd, v.amount_usd_min, v.amount_usd_max, v.amount_type, v.code_section, v.applies_to_track,
       '2024-01-09'::date, 'stated_on_document', fee_doc.id, v.notes
