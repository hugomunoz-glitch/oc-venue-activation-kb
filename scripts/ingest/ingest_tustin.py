"""
Phase 4, Tustin: reads chunks_tustin.py, embeds each chunk, writes
ingest_tustin.sql (documents/chunks + fee_schedules).

Usage:
    source .venv/bin/activate
    python ingest_tustin.py
    psql "$DATABASE_URL" -f ingest_tustin.sql
"""

import sys

from chunks_tustin import CITY, DOCUMENTS, FEE_ROWS
from common import build_document_sql, sql_literal

SOURCE_KEY_BY_FEE_NAME = {
    "Temporary Use Permit": "https://www.tustinca.org/DocumentCenter/View/633/Temporary-Use-Permit--General-PDF",
    "Conditional Use Permit (Planning Commission)": "https://www.tustinca.org/DocumentCenter/View/19836/2026-Comprehensive-Schedule-of-Fees",
    "Variance (Planning Commission)": "https://www.tustinca.org/DocumentCenter/View/19836/2026-Comprehensive-Schedule-of-Fees",
}
    ...
    for row in FEE_ROWS:
        value_rows.append(
            "(" + ", ".join([
                ...
                sql_literal(SOURCE_KEY_BY_FEE_NAME[row["fee_name"]]),
            ]) + ")"
        )
