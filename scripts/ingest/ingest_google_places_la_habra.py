"""
Phase 4 API ingestion: La Habra wedding-venue and gym listings from Google
Places API (New). Mirrors ingest_google_places_brea.py's structure and
address-verification filter.

Usage:
    source .venv/bin/activate
    python ingest_google_places_la_habra.py
    psql "$DATABASE_URL" -f ingest_google_places_la_habra.sql
"""
...
SEARCH_QUERIES = [
    "wedding venue La Habra CA",
    "gym La Habra CA",
    "banquet hall event venue La Habra CA",
]
...
""")
    ...
    out_path = os.path.join(os.path.dirname(__file__), "ingest_google_places_la_habra.sql")
