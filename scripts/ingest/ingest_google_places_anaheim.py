"""
Phase 4 API ingestion: pulls Anaheim wedding-venue and gym listings from
Google Places API (New), same pattern as ingest_google_places.py (Fullerton,
Phase 2) but for the second city. Kept as its own file rather than
generalizing the working Fullerton script, per this project's established
one-script-per-source-per-city pattern.

Usage:
    source .venv/bin/activate
    python ingest_google_places_anaheim.py
    psql "$DATABASE_URL" -f ingest_google_places_anaheim.sql
"""
...
SEARCH_QUERIES = [
    "wedding venue Anaheim CA",
    "gym Anaheim CA",
    "banquet hall event venue Anaheim CA",
]
...
def main():
    ...
    for query in SEARCH_QUERIES:
        ...
        for place in search_places(query):
            ...
            # Google's text search radius pulls in neighboring cities even
            # when the query names Anaheim explicitly (same issue Phase 3's
            # Zola pull hit) - only keep places whose address actually says
            # Anaheim, don't trust the query alone.
...
""")
    ...
    out_path = os.path.join(os.path.dirname(__file__), "ingest_google_places_anaheim.sql")
