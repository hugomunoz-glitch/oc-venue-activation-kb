"""
Ingests Brea venue pricing (chunks_brea_pricing.py). Mirrors
ingest_anaheim_pricing.py's structure for a single-venue dataset - Brea has
essentially no dedicated wedding-venue inventory of its own, so this is one
real, address-verified document + revenue_benchmarks row, not padded out.

Usage:
    source .venv/bin/activate
    python ingest_brea_pricing.py
    psql "$DATABASE_URL" -f ingest_brea_pricing.sql
"""

import os
import sys

from chunks_brea_pricing import CITY, DOCUMENTS, VENUES
...
def main():
    ...
    out_path = os.path.join(os.path.dirname(__file__), "ingest_brea_pricing.sql")
