"""
Ingests Zola venue pricing (chunks_zola.py): one document+chunk per venue
(for hybrid_search/citations) plus a structured revenue_benchmarks row per
venue (for the future break-even calculator).

Usage:
    source .venv/bin/activate
    python ingest_zola.py
    psql "$DATABASE_URL" -f ingest_zola.sql
"""

import os
import sys

from chunks_zola import CITY, DOCUMENTS, VENUES
...
select target_city.id, v.venue_name, v.venue_type, v.activation_type, v.starting_price_usd, v.price_note, v.guest_capacity_min, v.guest_capacity_max, v.service_tier,
       'medium', '2026-08-20'::date, (select id from documents where source_key = v.source_key), null
...
def main():
    ...
    out_path = os.path.join(os.path.dirname(__file__), "ingest_zola.sql")
