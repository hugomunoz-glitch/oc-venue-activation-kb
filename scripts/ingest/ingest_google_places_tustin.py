"""
Phase 4 API ingestion: Tustin wedding-venue and gym listings from Google
Places API (New). Mirrors the established address-verification filter.

Usage:
    source .venv/bin/activate
    python ingest_google_places_tustin.py
    psql "$DATABASE_URL" -f ingest_google_places_tustin.sql
"""
...
SEARCH_QUERIES = [
    "wedding venue Tustin CA",
    "gym Tustin CA",
    "banquet hall event venue Tustin CA",
]
...
""")
    ...
    out_path = os.path.join(os.path.dirname(__file__), "ingest_google_places_tustin.sql")
