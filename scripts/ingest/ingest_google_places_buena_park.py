"""
Phase 4 API ingestion: Buena Park wedding-venue and gym listings from
Google Places API (New). Mirrors the established address-verification
filter.

Usage:
    source .venv/bin/activate
    python ingest_google_places_buena_park.py
    psql "$DATABASE_URL" -f ingest_google_places_buena_park.sql
"""
...
SEARCH_QUERIES = [
    "wedding venue Buena Park CA",
    "gym Buena Park CA",
    "banquet hall event venue Buena Park CA",
]
...
""")
    ...
    out_path = os.path.join(os.path.dirname(__file__), "ingest_google_places_buena_park.sql")
