"""
Phase 4 API ingestion: Brea wedding-venue and gym listings from Google
Places API (New). Mirrors ingest_google_places_anaheim.py's structure and,
critically, its address-verification filter - the same cross-city
contamination bug found in Phase 2 (and re-confirmed for Anaheim) applies
here too.

Usage:
    source .venv/bin/activate
    python ingest_google_places_brea.py
    psql "$DATABASE_URL" -f ingest_google_places_brea.sql
"""
...
SEARCH_QUERIES = [
    "wedding venue Brea CA",
    "gym Brea CA",
    "banquet hall event venue Brea CA",
]
...
""")
    ...
    out_path = os.path.join(os.path.dirname(__file__), "ingest_google_places_brea.sql")
