"""
Phase 4, Buena Park: reads chunks_buena_park.py, embeds each chunk, writes
ingest_buena_park.sql (documents/chunks + fee_schedules).

Usage:
    source .venv/bin/activate
    python ingest_buena_park.py
    psql "$DATABASE_URL" -f ingest_buena_park.sql
"""

import sys

from chunks_buena_park import CITY, DOCUMENTS, FEE_ROWS
from common import build_document_sql, sql_literal

SOURCE_KEY_BY_PERMIT_TYPE = {
    "special_event_permit": "https://www.buenapark.com/departments/Building/TEMPORARY USE PERMIT APPLICATION.pdf",
    "conditional_use_permit": "https://cms7files1.revize.com/buenaparkca/Document_center/City%20Departments/Community%20development/Form/PLANNING%20DIVISION%20SCHEDULE%20OF%20FEES%202022.pdf",
}
...
select target_city.id, v.permit_type, v.fee_name, v.amount_usd, v.amount_usd_min, v.amount_usd_max, v.amount_type, v.code_section, v.applies_to_track,
       '2022-07-01'::date, 'stated_on_document', (select id from documents where source_key = v.source_key), v.notes
...
def main():
    ...
    out_path = "ingest_buena_park.sql"
