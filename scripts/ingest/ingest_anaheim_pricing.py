"""
Ingests Anaheim venue pricing (chunks_anaheim_pricing.py): one document+chunk
per venue (for hybrid_search/citations) plus a structured revenue_benchmarks
row per venue. Mirrors ingest_zola.py's structure for Fullerton.

Usage:
    source .venv/bin/activate
    python ingest_anaheim_pricing.py
    psql "$DATABASE_URL" -f ingest_anaheim_pricing.sql
"""

import os
import sys

from chunks_anaheim_pricing import CITY, DOCUMENTS, VENUES
...
def main():
    ...
    out_path = os.path.join(os.path.dirname(__file__), "ingest_anaheim_pricing.sql")
