"""
Phase 4 API ingestion: Santa Ana wedding-venue and gym listings from Google
Places API (New). Mirrors the established address-verification filter.

Usage:
    source .venv/bin/activate
    python ingest_google_places_santa_ana.py
    psql "$DATABASE_URL" -f ingest_google_places_santa_ana.sql
"""
...
SEARCH_QUERIES = [
    "wedding venue Santa Ana CA",
    "gym Santa Ana CA",
    "banquet hall event venue Santa Ana CA",
]
...
""")
    ...
    out_path = os.path.join(os.path.dirname(__file__), "ingest_google_places_santa_ana.sql")
